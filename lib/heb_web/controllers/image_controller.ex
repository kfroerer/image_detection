defmodule HebWeb.ImageController do
  ...
  def create(conn, %{"image_url" => image_url} = params) do
    ...
    with {:ok, image} <- Images.create_image(%{uri: image_url, label: label}) do
      handle_created_image(conn, image)
    else
      {:error, error} ->
        conn
        |> put_status(:unprocessable_entity)
        |> put_view(json: HebWeb.ChangesetJSON)
        |> render(:error, changeset: error)
    end
  end

  def create(conn, %{"image_file" => _file_path, "detect_objects" => true}) do
    # Heb.Imagga.get_image_tags_from_file(file_path)
    text(conn, "WIP")
  end

  def create(conn, %{"image_file" => _file}) do
